function [SDR,SIR,SAR,rec_test] = sep_with_framework(Tr,test,X,param,seeds)

k_s = [];
iter = 100;
while(isempty(k_s))
k_s = bin_search_dim(Tr,iter,param,seeds);     % dimension for source model
...
d(1) = k_s;
D_s = nmf_bases(Tr{1},k_s,iter,seeds.b1,seeds.g1);    % final NMF dictionary for the source
...
for disc_dim = disc_dim_vec(1):disc_dim_vec(2)-disc_dim_vec(1):disc_dim_vec(end)
    k = k+1;
    rng(seeds.b2);
    D_n = rand(size(Tr{2},1),disc_dim);
    rng(seeds.g2);
    C_nn = rand(disc_dim, size(Tr{2},2));
    for iter = 1:100
        D_n = update_B(D_n, C_nn, Tr{2}, D_s, param.lambda);
        C_nn = update_G(D_n, C_nn, Tr{2});
    end
    
 D = [D_s, D_n];
 
 C_s = rand(size(D,2), size(Tr{1},2));
 C_n = rand(size(D,2), size(Tr{2},2));
 for i = 1:100
     C_s = update_G(D,C_s,Tr{1});
     C_n = update_G(D,C_n,Tr{2});
 end
 
ener1(1,k) = (norm(D_s*C_s(1:size(D_s,2),:),'fro')^2)/(size(Tr{1},2));
ener1(2,k) = (norm(D_n*C_s(size(D_s,2) + 1:end ,:),'fro')^2)/(size(Tr{1},2));
ener2(1,k) = (norm(D_n*C_n(size(D_s,2) + 1:end ,:),'fro')^2)/(size(Tr{2},2));
ener2(2,k) = (norm(D_s*C_n(1:size(D_s,2),:),'fro')^2)/(size(Tr{2},2));
...
D_n = []; C_nn = [];
rng(seeds.b2);
D_n = rand(size(Tr{2},1),d(2));
rng(seeds.g2);
C_nn = rand(d(2), size(Tr{2},2));
for iter = 1:100
    D_n = update_B(D_n, C_nn, Tr{2}, D_s, param.lambda);
    C_nn = update_G(D_n, C_nn, Tr{2});
end
